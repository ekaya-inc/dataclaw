...
The UI lives in `ui/` (React 18 + Vite + Tailwind + react-router + CodeMirror). The core flow has three setup pages — Datasource, Approved Queries, Agent Access — plus a conditional "Support" page that appears in the sidebar only after Agent Access is configured and can be permanently dismissed per-browser. There is no auth. Don't add new top-level pages or an auth flow without an explicit product-level change. Built assets are checked into `internal/uifs/dist/` so the Go binary is self-contained; `make run` (or `make build-ui`) keeps them fresh.
