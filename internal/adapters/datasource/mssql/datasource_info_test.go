...
func newDatasourceInfoTestDB(t *testing.T, state *datasourceInfoDriverState) *sql.DB {
	...
	driverName := fmt.Sprintf("mssql-datasource-info-test-%d", atomic.AddUint64(&datasourceInfoDriverCounter, 1))
...
func TestDatasourceIntrospectorReturnsRuntimeInformation(t *testing.T) {
	state := &datasourceInfoDriverState{
		...
		columnTypes: []string{"NVARCHAR", "NVARCHAR", "NVARCHAR", "NVARCHAR"},
		rows: [][]driver.Value{{
			"warehouse",
			"dbo",
			"sa",
	...
	if info.DatabaseName != "warehouse" || info.SchemaName != "dbo" || info.CurrentUser != "sa" {
		t.Fatalf("unexpected datasource info: %#v", info)
	}
	if info.Version == "" {
		t.Fatalf("expected version to be populated, got %#v", info)
	}
	if len(state.querySQLs) != 1 || state.querySQLs[0] != `SELECT DB_NAME(), SCHEMA_NAME(), SUSER_SNAME(), @@VERSION` {
